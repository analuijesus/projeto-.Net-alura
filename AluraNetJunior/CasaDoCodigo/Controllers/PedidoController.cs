...
namespace CasaDoCodigo.Controllers
{
    public class PedidoController : Controller
    {
        ...
        public async Task<IActionResult> BuscaDeProdutos(string pesquisa)
        {
            ...
            {
                return View(new BuscaDeProdutoViewModel(await produtoRepository.GetProdutos(), true));
            }

            if (produtoRepository.GetProdutos(pesquisa).Result.Count() == 0)
            {
                return View(new BuscaDeProdutoViewModel(false));
            }

            return View(new BuscaDeProdutoViewModel(await produtoRepository.GetProdutos(pesquisa), true));
        }
    }
}
